...
fieldnames = ["id", "title", "release_date", "genres", "runtime", "vote_average", "vote_count", "director", "actor_1", "actor_2", "actor_3", "overview"]
...
for page in range(1, 11):
    ...
    for movie in movies:
        ...
        if movie_details:
            ...
            # Get top 3 cast members (split into separate columns)
            cast = movie_details.get("credits", {}).get("cast", [])[:3]
            actors = [actor["name"] for actor in cast]
            actor_1 = actors[0] if len(actors) > 0 else "N/A"
            actor_2 = actors[1] if len(actors) > 1 else "N/A"
            actor_3 = actors[2] if len(actors) > 2 else "N/A"
