...
required_columns = ['title', 'overview', 'tagline', 'keywords', 'genre_1', 'genre_2', 'genre_3', 'genre_4', 'genre_5', 'genre_6', 'vote_average', 'popularity', 'budget', 'revenue', 'actor_1', 'actor_2', 'actor_3', 'actor_4', 'actor_5', 'actor_6', 'producer_1', 'producer_2', 'producer_3', 'collection', 'original_language']
...
def clean_and_extract_columns(df):
    ...
    # Replace NaN values with zero in budget/revenue
...
# Preprocess Keywords with TF-IDF (Textual Features)
vectorizer = TfidfVectorizer(stop_words='english', max_features=1000)
keywords_matrix = vectorizer.fit_transform(df['keywords'].fillna(''))

# Preprocess Tagline with TF-IDF (Textual Features)
...
# Process collections as a binary feature (1 if in the same collection, 0 if not)
df['same_collection'] = df['collection'].apply(lambda x: 1 if pd.notna(x) else 0)
collection_matrix = df[['same_collection']].values

# Process original language as a categorical feature
language_matrix = pd.get_dummies(df['original_language'])

# Combine all features (textual features + binarized categorical features + numerical features)
combined_features = np.hstack([overview_matrix.toarray(), tagline_matrix.toarray(), keywords_matrix.toarray(), genres_matrix, actors_matrix, producers_matrix, numerical_data_scaled, collection_matrix, language_matrix])
