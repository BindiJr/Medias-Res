...
START_PAGE = 1  # The page to start from
NUM_PAGES = 50  # Process 50 pages at once, you can adjust this as needed
...
def get_processed_movie_ids():
    """Get the list of already processed movie IDs from the CSV file."""
    processed_ids = set()
    try:
        with open(CSV_FILE, mode="r", newline="", encoding="utf-8-sig") as file:
            reader = csv.DictReader(file)
            for row in reader:
                processed_ids.add(row["id"])  # Track movie IDs
    except FileNotFoundError:
        return processed_ids  # Return empty set if CSV doesn't exist yet
...
def process_movies(start_page, num_pages):
    """Main function to process movies in chunks."""
    processed_ids = get_processed_movie_ids()  # Get the list of processed movie IDs
    ...
    # Process movies in the specified range of pages
    while current_page < start_page + num_pages:
        ...
        for movie in movies:
            ...
            # Skip if the movie ID has already been processed
            if movie_id in processed_ids:
                print(f"Skipping movie {movie['title']} (ID {movie_id} already processed)")
                continue
            ...
            if movie_details:
                ...
                processed_ids.add(movie_id)  # Add to processed IDs set
                fetched_movies += 1
...
# Start processing
process_movies(START_PAGE, NUM_PAGES)
